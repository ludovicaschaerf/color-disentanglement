...
def main():
    ...
    kwargs = {'CL method':['LR'], 'C':[0.1, 1], 
              'max_lambda':[args.max_lambda], 'extremes':[True]}
    ...
    disentanglemnet_exp = InterfaceGAN(model, annotations, space='w', 
        ...
        for c in kwargs['C']:
            for extr in kwargs['extremes']: 
                ...
                for i in range(separation_vectors.shape[0]):
                    ...
                    name = 'InterfaceGAN_' + str(met) + '_' + str(c) + '_' + str(len(features)) + '_' + str(args.variable) + '_' + str(extr)
                    data.append([features[i], args.variable, 'w', name, args.subfolder, performance, ', '.join(features), str(separation_vectors[i, :])])
    ...
    df = pd.DataFrame(data, columns=['Feature', 'Variable', 'Space', 'Method', 'Subfolder', 'Performance', 'Classes', 'Separation Vector'])
